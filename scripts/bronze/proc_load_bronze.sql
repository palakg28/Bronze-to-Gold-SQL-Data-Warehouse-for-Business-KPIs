...
Script Purpose:
    ...
    external csv files and performs the following functions:
    - Truncates the bronze tables before loading the data
    - Uses the BULK INZSERT command to load data from csv files to bronze tables
...
BEGIN
    --Track ETL Duration - Helps identify bottlenecks, optimize performance,
    -- monitor trens, and detect issues
    DECLARE @start_time DATETIME, @end_time DATETIME, @batch_start_time DATETIME, 
    @batch_end_time DATETIME;

    --Track whole load duration
    SET @batch_start_time = GETDATE();
    -- Try cache added to handle bugs and error
    BEGIN TRY
   --FIRST MAKE AN EMPTY TABLE AND THEN INSERT VALUES
    ...
    WITH(
        -- This means that we have already defined the column names(first row)
        -- while creating the table so now we start with the second 
        --row(the entries)
        FIRSTROW = 2,
        FIELDTERMINATOR = ',',
        TABLOCK
